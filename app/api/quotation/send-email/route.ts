...
export async function POST(req: NextRequest) {
  try {
    ...
    const cleanClientName = (clientName || 'Client').trim();
    const pdfAttachmentName = `Proposal for ${cleanClientName}.pdf`;
    const emailSubject = subject || 'Proposal & Quotation — VOWS';
    ...
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 620px; margin: 0 auto; padding: 28px; color: #111a24; background-color: #ffffff; border: 1px solid #e6e8eb; border-radius: 6px;">
        <div style="border-bottom: 2px solid #111a24; padding-bottom: 12px; margin-bottom: 20px;">
          <h1 style="font-family: Georgia, serif; font-size: 24px; font-weight: 900; letter-spacing: 1px; text-transform: uppercase; margin: 0; color: #111a24;">VOWS</h1>
          <p style="font-size: 11px; text-transform: uppercase; letter-spacing: 2.5px; color: #738079; margin: 4px 0 0 0; font-family: monospace;">Wedding Cinematics & Stills</p>
        </div>

        <p style="font-size: 14px; line-height: 1.6; margin-bottom: 16px;">Dear <strong>${cleanClientName}</strong>,</p>
        
        <p style="font-size: 14px; line-height: 1.6; color: #333d47; white-space: pre-line; margin-bottom: 20px;">
          ${customMessage || 'Thank you for reaching out to VOWS. Please find attached our bespoke proposal and quotation for your celebration.'}
        </p>
        ...
        <div style="background-color: #f7f9fa; border: 1px solid #dce2e6; border-left: 4px solid #111a24; padding: 16px 20px; margin: 24px 0; border-radius: 3px;">
          <table style="width: 100%; font-size: 13px; font-family: monospace;">
            <tr>
              ...
              <td style="padding: 4px 0; text-align: right; font-weight: bold; color: #111a24;">${quotationNumber || 'VOWS-QUOTE'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; color: #6b7785; text-transform: uppercase;">Selected Package:</td>
              <td style="padding: 4px 0; text-align: right; font-weight: bold; color: #111a24;">${packageTitle || 'Bespoke Wedding Story'}</td>
            </tr>
            <tr style="border-top: 1px dashed #cfd7de;">
              ...
              <td style="padding: 10px 0 4px 0; text-align: right; font-weight: 900; font-size: 16px; color: #111a24;">Rs. ${Number(totalPrice || 0).toLocaleString('en-IN')}/-</td>
            </tr>
          </table>
        </div>

        <p style="font-size: 12px; color: #667085; line-height: 1.5; margin-bottom: 24px;">
          📎 <strong>Attachment:</strong> The official bespoke proposal & schedule document has been attached to this email as a PDF.
        </p>
        ...
        <div style="border-top: 1px solid #e6e8eb; padding-top: 18px; font-size: 12px; line-height: 1.5; color: #55606e;">
          ...
          <p style="margin: 2px 0;">Director & Lead Photographer // VOWS</p>
          <p style="margin: 2px 0;">Direct WhatsApp / Mobile: <strong>+91 93800 57445</strong></p>
    ...
    if (gmailUser && gmailPass) {
      ...
      const info = await transporter.sendMail({
        from: `"VOWS // Reuben Serrao" <${gmailUser.trim()}>`,
        to,
    ...
    if (resendApiKey) {
      const fromAddress = process.env.EMAIL_FROM || 'VOWS <onboarding@resend.dev>';
    ...
    return NextResponse.json({
      ...
      mailtoFallback: `mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(
        ...
        `Dear ${cleanClientName},\n\nPlease find attached the proposal for your celebration.\nTotal: Rs. ${Number(totalPrice || 0).toLocaleString('en-IN')}/-\n\nWarm regards,\nReuben Serrao\nVOWS\n+91 93800 57445`
      )}`,
    });
  } catch (error: any) {
